  ...
  function renderDevice(dev) {
    ...
    if (dev.lcd && dev.lcd.gif_supported) {
      ...
      lcd = `<div class="dx-rgb-lcd">
        <div class="dx-rgb-zone-lbl">LCD ${dev.lcd.width}×${dev.lcd.height} <span class="dx-rgb-lcd-badge">LCD Studio</span></div>
        <div class="dx-rgb-lcd-preview${round ? '' : ' square'}" id="lcd-prev-${esc(dev.id)}"><span class="muted fs-xs">${esc(dev.lcd.width)}×${esc(dev.lcd.height)}</span></div>
        <p class="muted fs-xs">GIF / video apply moved to <a href="#dx-lcd-studio" class="dx-rgb-lcd-studio-link">LCD Studio</a> (fit modes, library, honesty).</p>
