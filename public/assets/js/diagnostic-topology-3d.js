...
(function () {
  ...
  function ensurePopover() {
    ...
    popoverEl.className = 'dx-topology-node-popover';
    popoverEl.hidden = true;
    document.body.appendChild(popoverEl);
    return popoverEl;
  }

  function bindNodeClicks(domEl, topology3d) {
    if (!domEl || !window.THREE) return;
    ...
    domEl.addEventListener('pointermove', (ev) => {
      ...
      const node = (topology3d.nodes || []).find((n) => n.id === ud.id) || {};
      pop.innerHTML = `<strong>${ud.label || ud.id || 'Node'}</strong>
        <div>Type: ${ud.type || '—'}</div>
        <div>Temp: ${Math.round(lastTemps[ud.type === 'cpu' ? 'cpu' : 'gpu'] || lastTemps.gpu)}°C</div>
        ${node.detail ? `<div class="muted fs-xs">${node.detail}</div>` : ''}`;
      pop.style.left = `${ev.clientX + 12}px`;
      pop.style.top = `${ev.clientY + 12}px`;
      pop.hidden = false;
    });
    domEl.addEventListener('pointerleave', () => { pop.hidden = true; });
    domEl.addEventListener('click', (ev) => {
      const rect = domEl.getBoundingClientRect();
      pointer.x = ((ev.clientX - rect.left) / rect.width) * 2 - 1;
      pointer.y = -((ev.clientY - rect.top) / rect.height) * 2 + 1;
      raycaster.setFromCamera(pointer, camera);
      ...
      if (type === 'gpu' || type === 'cpu') {
        window.dispatchEvent(new CustomEvent('dx:nav-hardware', { detail: { focus: type } }));
        const btn = document.querySelector('[data-dx-nav="hardware"]') || document.querySelector('[data-dx-tab="hardware"]');
        if (btn) btn.click();
      }
    });
  }
  ...
  function initScene(root, topology3d) {
    ...
    syncThermDots();
    bindNodeClicks(renderer.domElement, topology3d);
