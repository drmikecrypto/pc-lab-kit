...
function Get-RgbEnableGuide {
    ...
    if (-not $HasOpenRgb) {
        $steps += 'OpenRGB Portable را در پوشه agent/pclab_probe/tools/OpenRGB/ قرار دهید - بدون نصب درایور، کنترل مستقیم از PcLab Probe.'
        $steps += 'PcLab Probe را یک‌بار Run as Administrator اجرا کنید تا دسترسی SMBus/USB برای LED فعال شود.'
    }
