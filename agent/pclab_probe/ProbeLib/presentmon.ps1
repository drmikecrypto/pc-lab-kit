...
function Get-PresentMonPercentileLows {
    ...
    # CapFrameX-style: mean of the lowest 1% / 0.1% FPS samples (sorted ascending).
    $sorted = @($FpsSamples | Sort-Object)
    $n = $sorted.Count
    $c1 = [Math]::Max(1, [int][Math]::Ceiling($n * 0.01))
    $c01 = [Math]::Max(1, [int][Math]::Ceiling($n * 0.001))
    if ($c1 -gt $n) { $c1 = $n }
    if ($c01 -gt $n) { $c01 = $n }
    $avg1 = ($sorted[0..($c1 - 1)] | Measure-Object -Average).Average
    $avg01 = ($sorted[0..($c01 - 1)] | Measure-Object -Average).Average
    return @{
        fps_1pct_low = [math]::Round($avg1, 1)
        fps_0_1pct_low = [math]::Round($avg01, 1)
    }
}

function Test-PresentMonSupportsV1Metrics {
    param([string]$Exe)
    if (-not $Exe) { return $false }
    if ($null -ne $script:PresentMonV1Metrics) { return [bool]$script:PresentMonV1Metrics }
    try {
        $help = & $Exe --help 2>&1 | Out-String
        $script:PresentMonV1Metrics = ($help -match 'v1_metrics')
    } catch {
        $script:PresentMonV1Metrics = $false
    }
    return [bool]$script:PresentMonV1Metrics
}

function Get-PresentMonCliBaseArgs {
    param([string]$Exe, [string]$OutputFile)
    $args = @(
        '--output_file', $OutputFile,
        '--delay', '0',
        '--no_summary'
    )
    if (Test-PresentMonSupportsV1Metrics -Exe $Exe) {
        $args += '--v1_metrics'
    }
    return $args
}
...
function ConvertFrom-PresentMonCsv {
    ...
    $headerRaw = [string]$lines[0]
    $headers = @($headerRaw -split ',' | ForEach-Object { ($_ -replace '^"|"$', '').Trim() })
    $idxApp = -1
    $idxMs = -1
    $idxFt = -1
    for ($h = 0; $h -lt $headers.Count; $h++) {
        $name = $headers[$h]
        if ($name -match '^(Application|ProcessName|Process)$') { $idxApp = $h }
        elseif ($name -match '^(MsBetweenPresents|msBetweenPresents)$') { $idxMs = $h }
        elseif ($name -match '^(MsBetweenDisplayChange|CPUBusy|GPUBusy)$') { }
        elseif ($name -match '(?i)frametime|msinpresentapi') { $idxFt = $h }
    }
    # PresentMon 1.x default layout fallback
    if ($idxApp -lt 0) { $idxApp = 0 }
    if ($idxMs -lt 0 -and $headers.Count -ge 9) { $idxMs = 8 }
    if ($idxFt -lt 0 -and $headers.Count -ge 10) { $idxFt = 9 }
    ...
    foreach ($line in $lines | Select-Object -Skip 1) {
        ...
        if ($idxApp -ge 0 -and $idxApp -lt $cols.Count) {
            $app = ($cols[$idxApp] -replace '^"|"$', '').Trim()
            if ($app -and $app -notmatch '^(Application|Process|ProcessName)$') {
                if (-not $appCounts.ContainsKey($app)) { $appCounts[$app] = 0 }
                $appCounts[$app]++
            }
        }
        $mb = 0.0
        if ($idxMs -ge 0 -and $idxMs -lt $cols.Count -and [double]::TryParse($cols[$idxMs], [ref]$mb) -and $mb -gt 0) {
            $msBetween += $mb
            $fpsSamples += (1000.0 / $mb)
            $frametimes += $mb
        }
        $ft = 0.0
        if ($idxFt -ge 0 -and $idxFt -lt $cols.Count -and $idxFt -ne $idxMs -and [double]::TryParse($cols[$idxFt], [ref]$ft) -and $ft -gt 0) {
            # Prefer dedicated frametime column when distinct from MsBetweenPresents
            if ($frametimes.Count -eq 0 -or $idxMs -lt 0) { $frametimes += $ft }
        }
    }
    $topApp = $null
    ...
    if ($fpsSamples.Count -eq 0 -and $frametimes.Count -gt 0) {
        $fpsSamples = @($frametimes | ForEach-Object { if ($_ -gt 0) { 1000.0 / $_ } else { 0 } })
        $msBetween = @($frametimes)
    }
    ...
    return @{
        ...
        process_name = $topApp
        label = $topApp
        methodology = '1%/0.1% lows = mean of lowest 1% / 0.1% FPS samples (CapFrameX-compatible)'
    }
}
...
function Get-ProbePresentMonTelemetry {
    ...
    try {
        $pmArgs = Get-PresentMonCliBaseArgs -Exe $exe -OutputFile $outCsv
        $pmArgs += @('--timed', "$TimedSeconds", '--terminate_after_timed')
        $null = Start-Process -FilePath $exe -ArgumentList $pmArgs -NoNewWindow -Wait -PassThru
...
function Start-ProbePresentMonSession {
    ...
    $args = Get-PresentMonCliBaseArgs -Exe $exe -OutputFile $paths.csv
...
function Add-PresentMonSpikeContext {
    param($Spikes, $Context, [double]$DurationMs = 0, [string]$StartedAt = '')
    ...
    $startUtc = $null
    if ($StartedAt) {
        try { $startUtc = [datetime]::Parse($StartedAt).ToUniversalTime() } catch {}
    }
    $sampleTimes = @()
    $hasTs = $false
    foreach ($s in $samples) {
        $ts = $null
        try {
            if ($s.t) { $ts = [datetime]::Parse([string]$s.t).ToUniversalTime() }
            elseif ($s.ts) { $ts = [datetime]::Parse([string]$s.ts).ToUniversalTime() }
        } catch {}
        if ($ts) { $hasTs = $true }
        $sampleTimes += $ts
    }
    ...
    foreach ($sp in $spikeList) {
        ...
        $tMs = [double]$row['t_ms']
        $idx = 0
        if ($hasTs -and $startUtc) {
            $target = $startUtc.AddMilliseconds($tMs)
            $best = [double]::MaxValue
            for ($si = 0; $si -lt $n; $si++) {
                if (-not $sampleTimes[$si]) { continue }
                $delta = [Math]::Abs(($sampleTimes[$si] - $target).TotalMilliseconds)
                if ($delta -lt $best) { $best = $delta; $idx = $si }
            }
        } elseif ($n -gt 1) {
...
function Save-ProbePresentMonSessionArtifact {
    ...
    $startAt = if ($Parsed.started_at) { [string]$Parsed.started_at } else { '' }
    $spikes = Add-PresentMonSpikeContext -Spikes $spikes -Context $context -DurationMs $durMs -StartedAt $startAt
