...
- Release builds: refresh `php_windows_url` in `config/build-deps.json` when windows.php.net rotates ZIPs; preflight and `bootstrap-build-tools.ps1` also try the **archives** URL. Since **4.2.4**, APP and desktop share one `MAJOR.MINOR.PATCH` (release-preflight asserts equality). Tauri rejects four-part versions. Before packaging the desktop shell, re-run `scripts/stage-desktop-payload.ps1` — `desktop/src-tauri/resources/lab/**` is gitignored and can go stale locally.
