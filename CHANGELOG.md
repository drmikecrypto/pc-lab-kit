...
### Changed
- Public repository reset: unified product version **0.0.7** (APP + desktop). Prior 4.x line remains in local archive history only.

## [4.2.5] - 2026-09-07

### Fixed — Lab hygiene & PresentMon hardening
- Timed PresentMon capture: `--output_file` + `--terminate_after_timed` (no bogus `CSV` stdout args / hang risk)
- CSV parse by header names (PresentMon 1.x/2.x); optional `--v1_metrics` when supported
- CapFrameX-style **mean** of lowest 1% / 0.1% FPS samples
- Spike↔telemetry: nearest sample by timestamp when ring has `ts`
- Probe PowerShell encoding normalized for Windows PS 5.1 (`scripts/fix-ps1-encoding.ps1`)
- Release preflight: **APP_VERSION == desktop** MAJOR.MINOR.PATCH
- RGB device cards: LCD GIF upload redirects to **LCD Studio** (legacy `/rgb/lcd` kept on probe)
